...
public class UserRelatedObjectDaoImpl implements IUserRelatedObjectDao{
    private HibernateTemplate hibernateTemplate;
	 

	public List<UserRelatedSentence> getByUser(final int userID,final int page,final int limit) {
		final String sql="from UserRelatedSentence as us " +
				"  where us.user.userID=:userID " +
				"  order by isActive desc,creatTime asc ";
	...
	public int getActiveRelatedDiariesSize(final long userID) {
		final String sql="select count(*) from UserRelatedObject u " +
				    "     where u.user.userID=:userID and u.objectType=1 and u.isObjectActive=1 " +
	...
	public int getActiveRelatedSentencesSize(final long userID) {
		final String sql="select count(*) from UserRelatedObject u " +
				    "     where u.user.userID=:userID and u.objectType=0 and u.isObjectActive=1 " +
				    "     order by u.createTime desc ";
			Long size=(Long) this.hibernateTemplate.executeFind(new HibernateCallback(){
	...
	public List<UserRelatedSentence> getRelatedDiaries(final long userID, final int page,
			final int limit) {
		final String sql="from UserRelatedObject u " +
				    "     where u.user.userID=:userID and u.objectType=1 and u.isObjectActive=1 " +
				    "     order by u.createTime desc ";
			return this.hibernateTemplate.executeFind(new HibernateCallback(){
