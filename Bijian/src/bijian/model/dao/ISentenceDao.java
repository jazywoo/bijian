...
public interface ISentenceDao extends IBaseDao {
   //get方法   得到不同条件下的句子
   public List<Sentence> getByUserID(long userID,int page,int limit) ;//特定用户的句子
   public List<Sentence> getByUserID(long userID,Date date1,Date date2,int page,int limit) ;//特定用户时间段的句子
  
   public List<Sentence> getAttention(List<Long> attentionUserList,int page,int limit) ;//某一个用户的所有关注人的最新句子
   public List<Sentence> getRelated(List<Long> relatedUserList,int page,int limit) ;//与之相关的句子
